...
class RecupPassActivity : AppCompatActivity() {
        ...
        auth?.sendPasswordResetEmail(edit_mail.text.toString())
            ?.addOnCompleteListener {
                if (it.isSuccessful){
                    ...
                    Toast.makeText(this@RecupPassActivity, "Enviar Email da Confirmação", Toast.LENGTH_SHORT).show()
                }else{

                    Toast.makeText(this@RecupPassActivity, "Error", Toast.LENGTH_SHORT).show()
                }
            }
    ...
    fun voltarLogin(view: View){
        ...
        val intent = Intent(this@RecupPassActivity, LoginActivity::class.java)
